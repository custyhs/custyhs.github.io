...
- **Reinforcement learning with function approximation**: developing theory for large state-action spaces beyond the tabular setting, where function classes are used to generalize across states and actions while preserving sample-efficiency and finite-time guarantees. Representative works include [Reward-Free Linear RL](https://openreview.net/forum?id=U9HW6vyNClg), [Risk Sensitive Linear RL](https://arxiv.org/abs/2605.06240), and [Risk Sensitive General Function Approximation](https://openreview.net/forum?id=0xmfExPqFf).
